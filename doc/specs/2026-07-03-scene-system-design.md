...
东方原作中，弹幕决斗结束后通常有一段"胜负已分后的交流"——信息交换、真相揭示、或者轻松的吐槽。这段对话是重要的叙事环节，场景应该配合营造"战斗结束、尘埃落定"的氛围。

**PostBattleSequence（战后序列）：**

Boss 战结束后不是立刻退场，而是经历一个完整的战后流程：

| 阶段 | 说明 |
|------|------|
| `Ceasefire` | 停火过渡——残余弹幕消散（复用现有 BulletClear），场景流动减速/停止，战斗紧张感逐渐消退 |
| `PostBattleDialogue` | 战后对话——使用 DialogueSceneMode 控制场景节奏。可以是 `Grounded`（双方落地面对面交谈）、`SlowScroll`（边走边聊的轻松感）、或 `FullStop`（严肃的信息交换） |
| `BossExit` | Boss 退场——对话结束后执行退场行为 |
| `SceneTransition` | 场景过渡——退场完成后，通路恢复流动/切换风格，进入下一阶段 |

**战后对话的场景表现：**

- `Grounded` 模式下：双方降落到地面，Boss 可以有受伤/疲惫的姿态变化（如单膝跪地、靠在障碍物上）
- 场景氛围随对话情绪变化：光照可以变柔和（战斗结束的释然）、粒子效果切换（弹幕残光 → 安静的落叶/尘埃）
- 镜头编排：对话镜头可以展示场景的战斗痕迹（被破坏的障碍物、弹坑），增强叙事厚度
- 对话结束时可以有"告别"动作——Boss 转身、挥手、或者做出某个标志性姿势

**BossExitBehaviour（Boss 退场行为）：**

退场发生在战后对话之后（如果有对话的话），或者直接在停火后触发：

| 模式 | 说明 |
|------|------|
| `RetreatToDistance` | Boss 向场景远处撤退，逐渐缩小消失（为后续再次出场留伏笔） |
| `WalkAway` | Boss 从容地走向障碍物深处/通路远方消失，体面的离场（适合友好/中立 Boss） |
| `CrashIntoScene` | Boss 坠落/撞向场景障碍物，引发场景破坏效果（适合被彻底击败的 Boss） |
| `DissolveWithScene` | Boss 与周围场景元素一起消散/变化，场景风格随之过渡（如竹林枯萎→新场景生长） |
| `FlyAway` | Boss 高速飞离画面（经典东方风格），可指定飞离方向 |
| `StayAsPresence` | Boss 不退场，而是转为 BossPresence 模式（如 `DistantFollow`），在后续道中继续作为场景元素存在——适合多阶段 Boss |
